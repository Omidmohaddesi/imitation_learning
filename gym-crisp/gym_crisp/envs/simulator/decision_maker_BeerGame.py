...
############ Added this for  four echelon level
class SimpleWSDecisionMaker(DecisionMaker):
    def __init__(self, ds):
        self.ws = ws

    def make_decision(self, now):
        inventory = self.ws.inventory_level()

        # allocated=allocate_proportional (self.ws)
        allocated = allocate_equally(self.ws)
        inventory -= allocated


        # Order Decision
        on_order = sum(tempO.amount for tempO in self.ws.on_order)
        backlog = self.ws.backlog_level()
        backlog -= allocated
        order_amount = max(self.ws.up_to_level + backlog -
                           on_order - inventory, 0)

        # Order Equally Recipe
        num_upstream_nodes = len(self.ws.upstream_nodes)
        for agent in self.ws.upstream_nodes:
            decision = OrderDecision()
            decision.upstream = agent
            decision.amount = int(float(order_amount) / num_upstream_nodes)
            self.ws.decisions.append(decision)
...
class SimpleMNDecisionMaker(DecisionMaker):
    ...
    def make_decision(self, now):
        ...
        prod_amount = self.mn.up_to_level + in_backlog - inventory - in_production # Yifan, i think this should be fixed ()production_start_rate[mn, t+1] = max(0, int(expected_orders[ds, t]+inventory_adjustment[mn, t] + supply_line_adjustment[mn, t]))
        if prod_amount < 0:
            prod_amount = 0
